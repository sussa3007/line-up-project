...
class EventServiceTest {
    ...
    @Test
    @DisplayName("검색조건으로 이벤트 검색하면, 검색된 결과물을 보여준다")
    void givenSearchParams_whenSearchingEvents_thenReturnEntreEventList() {
        // Given
        Long placeId = 1L;
        ...
        // When
        List<EventDTO> list =
                sut.getEvents(placeId, eventName, eventStatus, eventStartDatetime, eventEndDatetime);
        // Then
        assertThat(list)
                .hasSize(1)
                .allSatisfy(event -> {
    ...
    @Test
    @DisplayName("이벤트를 검색하는데 에러가 발생한 경우, 줄서기 프로젝트 기본 에러로 전환하여 예외 던지기")
    void givenDataException_whenSearchingEvents_thenReturnsGeneralException() {
        // Given
